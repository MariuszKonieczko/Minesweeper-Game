...
class Game extends UI {
  ...
  handleCellContextMenu = (e) => {
    ...
    const colIndex = parseInt(target.getAttribute('data-x'), 10);
    const cell = this.cells[rowIndex][colIndex];
    if (cell.isReval || this.isGameFisnished) return;
  ...
  clickCell(cell) {
    if (this.isGameFisnished || cell.isFlagged || cell.isReval) return;
    if (cell.isMine) {
      this.endGame(false);
      return;
    }
    ...
    if (this.revealdCells === this.cellsToReveal && !this.isGameFisnished) {
      this.endGame(true);
    }
  }
